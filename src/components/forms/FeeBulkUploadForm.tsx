...
export default function FeeBulkUploadForm({ onSuccess, onCancel }: { onSuccess: () => void, onCancel: () => void }) {
  ...
  const downloadSample = () => {
    ...
    XLSX.writeFile(wb, 'Special5_Fees_Sample.xlsx');
  };
    ...
    reader.onload = async (e) => {
      try {
        ...
        const formattedData = json.map((row: any) => ({
          ...
          month: row.month || new Date().toLocaleString('default', { month: 'short', year: 'numeric' }),
          payment_date: row.payment_date || new Date().toISOString(),
          payment_mode: row.payment_mode || 'Cash',
          paid: String(row.paid).toLowerCase() === 'true'
        }));

        const { error: uploadError } = await supabase.from('fees').insert(formattedData);
        ...
        setError(err.message || 'Failed to upload fees. Please check the format and ensure student IDs exist.');
      } finally {
        setLoading(false);
      }
    };
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: 'var(--surface-color)', padding: '1rem', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
        <div>
          ...
          <p style={{ fontSize: '0.75rem', color: 'var(--muted)', marginTop: '0.25rem' }}>Use this template to format your fee collection data.</p>
        </div>
      ...
      {successCount !== null && (
        <div style={{ padding: '1rem', backgroundColor: '#ecfdf5', color: '#059669', borderRadius: '8px', display: 'flex', alignItems: 'flex-start', gap: '0.5rem', fontSize: '0.875rem' }}>
          ...
          <span>Successfully imported {successCount} fee records! Redirecting...</span>
        </div>
      )}
